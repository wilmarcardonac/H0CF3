...
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+z_step) & (data[index,0] <= z_min+2.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[1],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+z_step)+'_z_max'+str(z_min+2.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+2.*z_step) & (data[index,0] <= z_min+3.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[2],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+2.*z_step)+'_z_max'+str(z_min+3.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+3.*z_step) & (data[index,0] <= z_min+4.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[3],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+3.*z_step)+'_z_max'+str(z_min+4.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+4.*z_step) & (data[index,0] <= z_min+5.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[4],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+4.*z_step)+'_z_max'+str(z_min+5.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+5.*z_step) & (data[index,0] <= z_min+6.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[5],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+5.*z_step)+'_z_max'+str(z_min+6.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+6.*z_step) & (data[index,0] <= z_min+7.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[6],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+6.*z_step)+'_z_max'+str(z_min+7.*z_step)+'_CF3.pdf')
py.close()
hp.graticule(dpar=10.,dmer=10.,coord='G')
for index in range(len(Dist)):
    if ( (data[index,0] > z_min+7.*z_step) & (data[index,0] <= z_min+8.*z_step) ):
        hp.projplot(data[index,2],data[index,3],marker[7],lonlat=True,coord='G')
py.savefig('../figures/angular_distribution_galaxies_redshift_bin_z_min_'+str(z_min+7.*z_step)+'_z_max'+str(z_min+8.*z_step)+'_CF3.pdf')
py.close()
exit()
