...
subroutine read_data_CF3(path_to_datafile)
  ...
  allocate (redshift(1:arrays_dimension),luminosity_distance(1:arrays_dimension),galactic_longitude(1:arrays_dimension),&
       galactic_latitude(1:arrays_dimension),redshift_corrected(1:arrays_dimension),stat=status1)
  ...
  Do p=1,arrays_dimension
     ...
     read(11,*) redshift(p),luminosity_distance(p),galactic_longitude(p),galactic_latitude(p),redshift_corrected(p)
...
subroutine compute_number_counts_map_corrected(zmin,zmax,jackknife,data_index_excluded,dip_amplitude,dip_latitude,dip_longitude,&
     noise_dip_amplitude,noise_dip_latitude,noise_dip_longitude)
  ...
  Do data_index=1,number_galaxies_in_CF3
     ...
     If ( (redshift_corrected(data_index) .gt. zmin) .and. (redshift_corrected(data_index) .le. zmax) ) then
...
end subroutine compute_number_counts_map_corrected
...
subroutine jackknife_analysis_corrected(zmin,zmax,current_amplitude,current_latitude,current_longitude,&
     noise_amplitude,noise_latitude,noise_longitude)
...
!  use head_fits
  ...
  Do data_index=1,number_galaxies_in_CF3
     ...
     If ( (redshift_corrected(data_index) .gt. zmin) .and. (redshift_corrected(data_index) .le. zmax) ) then
        ...
        mean_redshift = redshift_corrected(data_index) + mean_redshift
  ...
  Do index_jackknife_analysis=1,dimension_jackknife_analysis
     ...
     call compute_number_counts_map_corrected(zmin,zmax,.true.,jackknife_data_indices(index_jackknife_analysis),&
          jackknife_dipole_amplitude(index_jackknife_analysis),jackknife_galactic_latitude(index_jackknife_analysis),&
          jackknife_galactic_longitude(index_jackknife_analysis),jack_noise_dipole_amplitude(index_jackknife_analysis),&
          jack_noise_galactic_latitude(index_jackknife_analysis),jack_noise_galactic_longitude(index_jackknife_analysis))
...
end subroutine jackknife_analysis_corrected
